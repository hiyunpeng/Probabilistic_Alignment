...
def make_run_plan(budgets: List[int], suite: List[ProblemSpec], instances: int, seeds: int,
    ...
    for spec in suite:
        for inst in range(instances):
            shift_seed = 10_000 * (hash(spec.name) % 1000) + inst
            for budget in budgets:
                for algo in algos:
                    for r in range(seeds):
                        seed = 1_000_000 * (hash(algo) % 1000) + 10_000 * inst + 37 * budget + r
...
def analyze_and_report(summary_csv: Path, out_dir: Path, budgets: List[int],
                       ...
                       expected_groups: Dict[str, str], tau: float = 6.0) -> None:
    ...
    for b in budgets:
        for t in targets:
            ...
            for view in ["ABS", "REL"]:
                ...
                medoids = pick_family_medoids(D, algo_order, expected_groups)
                mem = soft_membership(D, algo_order, medoids, tau=tau)
                ...
                for algo, lab in zip(algo_order, labels):
                    cluster_rows.append({
                        ...
                        "expected_group": expected_groups.get(algo, "UNK"),
                        "anchor_PSO": medoids.get("PSO", ""),
                        "anchor_ES": medoids.get("ES", ""),
                    })
...
def parse_args():
    ...
    ap.add_argument("--algos", type=str, default="GWO,MFO,WOA,FA,BA,ALO", help="Comma-separated algos to test")
...
def main():
    ...
    if args.analysis_only:
        ...
        analyze_and_report(summary_csv, out_dir, budgets, targets, algos, EXPECTED_GROUPS_DEFAULT, tau=args.tau)
        return
