...
# -*- coding: utf-8 -*-
"""
dorigo_eval_only_from_best_params.py

Eval-only pipeline:
- Read best hyperparameters from a tuning folder that contains:
    <in_dir>/<ALGO>/best_params.json
- Run HOLDOUT evaluation (repeated runs across budgets/targets).
- Output run-level logs + aggregated instance_algo_budget_summary.csv + success-vs-budget plots.

This matches the output schema of dorigo_tune_then_eval_with_progress.py (eval part).

Dependencies:
  pip install numpy pandas scipy matplotlib tqdm

Example:
  python dorigo_eval_only_from_best_params.py \
    --in_dir out_dorigo_tuned/tuning \
    --out_dir out_dorigo_tuned/eval_only \
...
Notes:
- It re-creates the same benchmark instances and DEV/HOLDOUT split deterministically from seed/dev_frac.
- If you want to evaluate on ALL instances (no holdout), pass --dev_frac 0.
"""
...
# -----------------------------
# Algorithms (must match the tuning script)
# -----------------------------
...
# -----------------------------
# Targets + Beta-Binomial summaries
# -----------------------------
...
def load_best_params(in_dir: Path, algo: str, file_name: str) -> Dict[str, Any]:
    p = in_dir / algo / file_name
    if not p.exists():
        raise FileNotFoundError(f"Cannot find best params for {algo}: {p}")
    with p.open("r", encoding="utf-8") as f:
        data = json.load(f)
    if not isinstance(data, dict):
        raise ValueError(f"{p} must be a JSON object/dict, got: {type(data)}")
    return data
...
def parse_args():
    ...
    ap.add_argument("--in_dir", type=str, required=True,
                    help="Folder that contains <algo>/best_params.json for each algo.")
    ap.add_argument("--out_dir", type=str, required=True)

    ap.add_argument("--params_file", type=str, default="best_params.json",
                    help="Filename for params JSON under each algo folder.")
    ...
    ap.add_argument("--dev_frac", type=float, default=0.4,
                    help="Re-create same split; holdout = (1-dev_frac). Use 0 for all instances.")
...
def main():
    ...
    _dev, hold = split_instances(insts, args.dev_frac, args.seed)
    ...
    (out_dir / "README.txt").write_text(
        ...
        f"dev_frac={args.dev_frac} (holdout={len(hold)})\n"
    ...
    pb_load = tqdm(algos, desc="LOAD best_params", leave=True, disable=not progress_enable)
    for algo in pb_load:
        if hasattr(pb_load, "set_postfix_str"):
            pb_load.set_postfix_str(algo)
        params = load_best_params(in_dir, algo, args.params_file)
        params = coerce_params_for_budget(params, min(budgets))
        params_map[algo] = params

    # Evaluate
    all_rows = []
    pb_eval = tqdm(algos, desc="EVAL", leave=True, disable=not progress_enable)
    for algo in pb_eval:
        if hasattr(pb_eval, "set_postfix_str"):
            pb_eval.set_postfix_str(algo)
        rows = run_grid_for_algo(
            algo, params_map[algo], hold, args.dim, budgets, args.R_eval, base_seed=args.seed + 999,
            progress_desc=f"runs {algo}", progress_enable=progress_enable
        )
    ...
    inst_table.to_csv(out_dir / "instance_algo_budget_summary.csv", index=False)
