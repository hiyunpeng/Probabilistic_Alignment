...
Run the full HOLDOUT experiment with:
  ...
  - 4 anchor families: PSO_GBEST, PSO_RING, ES_1P1, ES_MULAMBDA
...
def run_pso_ring(f, dim, bounds, budget, seed, pop=30, w=0.72, c1=1.49, c2=1.49, neigh=1):
    """
    Ring-topology PSO: each particle is attracted to the best personal-best within a ring neighborhood.
    neigh=1 means {i-1,i,i+1}. Larger neigh widens the neighborhood.
    """
    rg = rng_from(seed)
    lo, hi = bounds
    X = rg.uniform(lo, hi, size=(pop, dim))
    ...
    # helper to compute local-best per particle from pbest/pbest_fit
    idx = np.arange(pop)
    # Precompute neighbor offsets
    offsets = np.arange(-neigh, neigh + 1)

    while evals + pop <= budget:
        # local best indices
        # Build neighbor index matrix: pop x (2*neigh+1)
        neigh_idx = (idx[:, None] + offsets[None, :]) % pop
        neigh_fit = pbest_fit[neigh_idx]  # pop x K
        best_j = np.argmin(neigh_fit, axis=1)  # pop
        lbest_idx = neigh_idx[np.arange(pop), best_j]  # pop
        lbest = pbest[lbest_idx]  # pop x dim
    ...
    # return best personal-best found
    best_idx = int(np.argmin(pbest_fit))
    return float(pbest_fit[best_idx]), evals
...
def run_es_mulambda(f, dim, bounds, budget, seed, mu=15, lam=60, sigma0: float = None):
    """
    Simple (mu, lambda)-ES with isotropic Gaussian mutation and a global step-size adapted by a 1/5-style rule
    on whether the generation best improves the global best.
    """
    rg = rng_from(seed)
    lo, hi = bounds

    parents = rg.uniform(lo, hi, size=(mu, dim))
    fit_p = eval_pop(f, parents)
    evals = mu

    best = float(np.min(fit_p))
    sigma = 0.2 * (hi - lo) if sigma0 is None else float(sigma0)
    ...
    while evals + lam <= budget:
        gen += 1
        # choose parents uniformly (comma selection)
        idx = rg.integers(0, mu, size=lam)
        base = parents[idx]
        off = base + sigma * rg.normal(0.0, 1.0, size=(lam, dim))
        off = clip_bounds(off, lo, hi)
        fit_o = eval_pop(f, off)
        evals += lam

        # select best mu offspring
        order = np.argsort(fit_o)
        parents = off[order[:mu]]
        fit_p = fit_o[order[:mu]]

        gen_best = float(fit_p[0])
        if gen_best < best:
            best = gen_best
            succ += 1
...
DEFAULT_PARAMS: Dict[str, Dict[str, Any]] = {
    ...
    "PSO_RING":  {"pop": 30, "w": 0.72, "c1": 1.49, "c2": 1.49, "neigh": 1},
    "ES_1P1":    {"pop": 30, "sigma0": None},
    "ES_MULAMBDA": {"mu": 15, "lam": 60, "sigma0": None},
}
